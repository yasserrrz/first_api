...
createServer((req: IncomingMessage, res: ServerResponse) => {
    if (req.method === "GET" && req.url === "/getUsers") {
       ...
       req.on("end" , ()=>{ 
        ...
        //        }else{
        //         res.write(JSON.stringify({message:"Please inter a value to update"}))
        //         res.end()
        //       }
        ...
        //     res.end()
        //    }else{
        //     res.write(JSON.stringify({message:"Please inter a correct id"}))
        //     res.end()

        //    }
        if (index !== -1) {
            if (updateUser.email) else if (updateUser.email) {
                users[index].email = updateUser.email;
            } else if (updateUser.name) {
                users[index].name = updateUser.name;
