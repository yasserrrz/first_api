      ...
      let convertingChunk : any ; 
       ...
       req.on("end" , ()=>{
        ...
        req.on("end", ()=>{
            ...
            let searchval :string[] = searchData.body.toLowerCase().split(" ")
            if(searchval){

                let seachLeangth : number  = searchval.length ; 
               let answer :Post[] = posts.filter((post) =>{
                return   searchval.some(word => post.title.toLowerCase().includes(word))
               })

               res.write(JSON.stringify({message : "success" , answer}))
            }else{
                res.write(JSON.stringify({message : "F"}))
            }
